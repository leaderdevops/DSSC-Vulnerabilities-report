...
def parse_args():
    ...
    parser.add_argument(
        '--exlchoice',
        action='store',
        help='These makes a selection of single sheet excel ot multiple sheet'
    )
    parser.add_argument(
        'image',
...
def generate_multiple_xls(data):
    import xlsxwriter
    ...
    if data['malware']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(row + 5, col, row + 5, col + 6, message,
                              error)
    ...
    if data['content_risk']['items']:
        ...
        message = "No-Record"
        worksheet.merge_range(tablesize2 + 5, col, tablesize2 + 5, col + 6, message,
                              error)
    tablesize3 = tablesize2 + 5
    ...
    if data['compliance_check_failures']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(tablesize3 + 5, col, tablesize3 + 5, col + 6, message,
                              error)
    ...
    if data['compliance_checklist']['pci-dss']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(tablesize4 + 5, col, tablesize4 + 5, col + 6, message,
                              error)
    ...
    if data['compliance_checklist']['nist800190']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(tablesize5 + 5, col, tablesize5 + 5, col + 6, message,
                              error)
    ...
    if data['compliance_checklist']['hipaa']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(tablesize6 + 5, col, tablesize6 + 5, col + 6, message,
                              error)
    ...
    if data['vulnerable_package']['items']:
        ...
        message = "No-Record Found"
        worksheet.merge_range(tablesize7 + 5, col, tablesize7 + 5, col + 6, message,
                              error)
...
def main():
    ...
    result = get_analysis(args.smartcheck_host, args.smartcheck_user, args.smartcheck_password, args.min_severity,
                          ...
                          args.show_overridden, args.exlchoice)
    print(result)
    print("vulnerable result  ", result['vulnerable_package']['items'])

    def switch(argument):
        switcher = {
            "single": generate_single_xls(result),
            "multiple": generate_multiple_xls(result)

            # "single": generate_single_xls(result),
            # "multiple": generate_multiple_xls(result),

        }
        return switcher.get(argument, "NO SELECT SINGLE OR MULTIPLE")

    if args.exlchoice == "single":
        generate_single_xls(result)
    elif args.exlchoice == "multiple":
        generate_multiple_xls(result)
    #print(switch(args.exlchoice))
